...
async function processVideo() {
  ...
  if (!videoId) {
    console.error('Falta especificar el id del video al ejecutar el comando')
    return
  }

  console.log('Consiguiendo nombre del video...')
  try {
    const paramsGetTitle = ['-e', `https://www.youtube.com/watch?v=${videoId}`]
    const title = spawnSync(`yt-dlp`, paramsGetTitle).output.toString()
    console.log('Nombre:', title)
  } catch {
    console.error('No se pudo conseguir el nombre de video')
  }
  ...
  try {
    ...
    await execPromise(`bun ${cwd()}/programas/serie/crear_resoluciones.ts ${videoId}`)
